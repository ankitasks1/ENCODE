...
parser.add_argument('--output', help='path of temporary files generated from merging sample of samplesheet 2',  required=True)
...
for values in Intersect:
    for values2 in Intersect[values]:
        ...
        with open(''.join(str(args.processed_merged_2) + args.output), 'a') as myoutfile:
            myoutfile.write(''.join(values + '\t' + values2 + '\t' + Original[values] + '\t' + Cleaned[values] + '\t' + Original[values2] + '\t' + Cleaned[values2] + '\t' + Intersect[values][values2] + '\n'))
            myoutfile.close()