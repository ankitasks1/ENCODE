...
### Peak based: Hot spopt regions were  identified by using approach in which peak's start and end is divided by window size and rounded-off to defined as occupancy:
...
### Summit based: Hot spopt regions were  identified by using approach in which summit's start and end is divided by window size and rounded-off to defined as occupancy (The summit bedfiles are already filtered and produced by nextflow):
<code> python makebins_from_summit_q.py --input fsample_list.txt --gbinsize 1000 --output gbin_1000_summit_encode_out.txt </code>
