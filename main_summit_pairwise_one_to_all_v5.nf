...
params.blacklist_2 = "$projectDir/dummy_blacklist_2.bed"
...
log.info """\
    ...
    Blacklist 1               : ${params.blacklist_1}
    Blacklist 2               : ${params.blacklist_2}
    Hotspotregions            : ${params.hotspotregions}
    """
    .stripIndent()
}
...
process COUNTFILTERED_1{
    ...
    script:
    """
    python3 ${params.scripts}summit_peakcounter.py --inputfile "${bedfile_1}_summit_freeblackhot_sorted_merged_1.bed" --location other --processed_file ${params.outdir_1} --type cleaned_1
...
process REMOVEBLACKLISTBED_2{
    ...
    script:
    """
    bedtools intersect -a ${params.outdir_2}${bedfile_2}_filt_2.bed -b $blacklist_2 -v >  ${bedfile_2}_filt_freeblack_2.bed
    """
}
...
process COUNTFILTERED_2{
    ...
    script:
    """
    python3 ${params.scripts}summit_peakcounter.py --inputfile "${bedfile_2}_summit_freeblackhot_sorted_merged_2.bed" --location other --processed_file ${params.outdir_2} --type cleaned_2
    python3 ${params.scripts}summit_peakcounter.py --inputfile "${bedfile_2}" --location current_2 --currentpath ${params.inputdir} --type original_2
    """
}
...
process COUNTCLOSESETWITHMERGED{
    tag "Count closest distance of one summit bed with summit merged files"
    publishDir params.outdir, mode: 'copy'
    input:
    file 'summit_distancecounter.py'
    path(pairs)

    output:
    val "${params.outdir}closest_distance_out.txt"
    ...
    BEDNAME_1=\$(basename "${pairs[0]}" "_peaks_id.bed_summit_freeblackhot_sorted_merged_1.bed")
    BEDNAME_2=\$(basename "${pairs[1]}" "_peaks_id.bed_summit_freeblackhot_sorted_merged_2.bed")
    BEDNAME_1=\$(echo "\${BEDNAME_1}" | cut -d '_' -f 1)
    BEDNAME_2=\$(echo "\${BEDNAME_2}" | cut -d '_' -f 1)
    CLOSEST=\$(bedtools closest -a "${params.outdir_1}/${pairs[0]}" -b "${params.outdir_2}/${pairs[1]}" -d | awk '{if(\$9 >= 0 && \$9 <= 150) print \$0}' | wc -l)
    echo "\${BEDNAME_1}\t\${BEDNAME_2}\t\${CLOSEST}" >> ${params.outdir}closest_distance_out.txt
    """
}
...
process WRITEOUTPUT{
    ...
    script:
    """
    python3 ${params.scripts}summit_poolcounts.py --currentpath ${params.inputdir} --processed_file_1 ${params.outdir_1} --processed_file_2 ${params.outdir_2} --processed_merged_2 ${params.outdir} --output ${params.finalout}
    """
}
...
workflow{
    ...
    blacklistfreebed_2_ch = REMOVEBLACKLISTBED_2(individual_beds_2_ch, cleanbedup_2_ch, params.blacklist_2)
    ...
    pair_ch
        .collect()
        .set {pair_collected_ch}
    
    // pair_collected_ch.view()

    countclosestmerged_ch = COUNTCLOSESETWITHMERGED(pair_collected_ch, pair_ch).collect()
    // countclosestmerged_ch.view()
    countclosestmerged_ch
        .collect()
        .set {outcountclosestmerged_ch}
    
    // // // count_filtered_1_ch.view()
    // // // count_filtered_2_ch.view()
    // // // outcountintersectmerged_ch.view()

    outputfinal_ch = WRITEOUTPUT(outcountclosestmerged_ch)
}
