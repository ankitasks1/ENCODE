...
usage example: python3 summit_poolcounts.py  --currentpath /Users/ankitverma/Documents/tutorial/nextflow/ENCODE/summit_pairwise/ --processed_file_1 /Users/ankitverma/Documents/tutorial/nextflow/ENCODE/summit_pairwise/allouts_1/ --processed_file_2 /Users/ankitverma/Documents/tutorial/nextflow/ENCODE/summit_pairwise/allouts_2/ --processed_merged_2 /Users/ankitverma/Documents/tutorial/nextflow/ENCODE/summit_pairwise/merged_out/ --output testfinalout.txt
''')
...
with open(''.join(args.processed_merged_2 + 'closest_distance_out.txt')) as myintersect:
...
with open(''.join(args.currentpath + 'original_1.count1.txt')) as original:
    ...
    for elements in original:
        ...
        Original[elements[0].replace('_peaks_id.bed', '').split('_')[0]] = elements[1]
...
with open(''.join(args.processed_file_1 + 'cleaned_1.count.txt')) as cleaned:
    ...
    for elements in cleaned:
        elements = elements.strip().split('\t')
        Cleaned[elements[0].replace('_peaks_id.bed_summit_freeblackhot_sorted_merged_1.bed', '').split('_')[0]] = elements[1]
...
with open(''.join(args.processed_file_2 + 'cleaned_2.count.txt')) as cleaned:
    ...
    for elements in cleaned:
        elements = elements.strip().split('\t')
        Cleaned[elements[0].replace('_peaks_id.bed_summit_freeblackhot_sorted_merged_2.bed', '').split('_')[0]] = elements[1]
